...
talks to external services (Stripe CLI relay, real card entry, real R2) — run it only when the
user asks for live verification, and never in CI.
...
Specs must stay self-contained: unique per-run names/emails, release any reservation they
create, and never assume the catalog holds only seed data.
